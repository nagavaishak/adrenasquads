...
import {
  getMutagenBoost,
  getSquadBoostsForCompetition,
} from './mutagen/integration.js';
import {
...
} from './analytics/abuse-detection.js';
...
app.get('/api/mutagen/boost/:wallet', async (req, res) => {
  try {
    const result = await getMutagenBoost(req.params.wallet);
...
app.get('/api/mutagen/squad-boosts/:competitionId', async (req, res) => {
  try {
    ...
    const squads = await getSquadBoostsForCompetition(competitionId);
    res.set('X-Integration-Note', MUTAGEN_NOTE);
    res.json({
      success: true,
...
app.get('/api/admin/abuse-report/:competitionId', async (req, res) => {
  ...
  try {
    ...
    const { rows: compRows } = await pool.query<{
      ...
      `SELECT competition_pubkey, start_time, end_time, status
       FROM competitions WHERE competition_id = $1 LIMIT 1`,
      [competitionId],
    );
    ...
    for (const row of memberRows) {
      ...
      if (!squad.members.includes(row.wallet)) squad.members.push(row.wallet);
    ...
    const { rows: predRows } = await pool.query<{
      ...
      `SELECT user_wallet, squad_picked, amount_staked, created_at
       FROM predictions WHERE competition_pubkey = $1`,
      [compPubkey],
    );

    const predictions: PredictionEntry[] = predRows.map(r => ({
      wallet:       r.user_wallet,
      squad_pubkey: r.squad_picked,
      amount:       Number(r.amount_staked) / 1_000_000,
      placed_at:    Math.floor(new Date(r.created_at).getTime() / 1000),
    }));

    // ── 5. Run detection functions ───────────────────────────────────────────
    const correlatedPairs: ReturnType<typeof detectCorrelatedTrading> = [];
    for (const squad of squads) {
      const pairs = detectCorrelatedTrading(squad.members, allTrades);
      correlatedPairs.push(...pairs.filter(p => p.flagged));
    }

    const washResults = Array.from(tradesByWallet.keys()).map(wallet =>
      detectWashTrading(wallet, allTrades),
    ).filter(r => r.flagged);

    const inactiveResults: ReturnType<typeof detectInactiveMembers> = [];
    const now = Math.floor(Date.now() / 1000);
    for (const squad of squads) {
      const inactive = detectInactiveMembers(squad, tradesByWallet, now);
      inactiveResults.push(...inactive.filter(r => r.flagged));
    }

    const frontRunResults = detectPredictionFrontRunning(predictions, poolLockTime)
      .filter(r => r.flagged);

    // ── 6. Risk score ────────────────────────────────────────────────────────
    const totalFlags =
      correlatedPairs.length + washResults.length +
    ...
    const parts: string[] = [];
    if (washResults.length)    parts.push(`${washResults.length} potential wash trader${washResults.length > 1 ? 's' : ''}`);
    if (correlatedPairs.length) parts.push(`${correlatedPairs.length} correlated pair${correlatedPairs.length > 1 ? 's' : ''}`);
    if (inactiveResults.length) parts.push(`${inactiveResults.length} inactive member${inactiveResults.length > 1 ? 's' : ''}`);
    if (frontRunResults.length) parts.push(`${frontRunResults.length} front-run prediction${frontRunResults.length > 1 ? 's' : ''}`);
...
function buildDemoAbuseReport(competitionId: number) {
  const now = Math.floor(Date.now() / 1000);
  return {
    competition_id:     competitionId,
