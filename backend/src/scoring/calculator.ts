...
export interface ClosedTrade {
  ...
  wallet: string;
  pnl: number;          // USD
  collateral: number;   // USD — collateral used for this trade
...
export function calculateMemberScore(
  ...
  const score =
    maxCollateral > 0
      ? Math.round((realizedPnl / maxCollateral) * 10_000)
      : 0;
...
export function calculateSquadScore(
  ...
  const totalScore = memberScores.reduce(
    (sum, m) => sum + (m.tradeCount > 0 ? m.score : 0),
    0,
  );
 ...
 */
export function rankSquads(squads: SquadScore[]): SquadScore[] {
 ...
 */
export function rankToChampionshipPoints(rank: number): number {
  if (rank === 1) return 100;
  if (rank === 2) return 85;
  if (rank === 3) return 70;
  if (rank === 4) return 60;
  if (rank === 5) return 50;
  if (rank <= 10) return 40 - (rank - 6) * 5; // 40, 35, 30, 25, 20
  return 10;
}
