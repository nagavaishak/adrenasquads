...
exports.ADRENA_PROGRAM_ID = void 0;
exports.getClosedTrades = getClosedTrades;
const web3_js_1 = require("@solana/web3.js");
exports.ADRENA_PROGRAM_ID = '13gDzEXCdocbj8iAiqrScGo47NiSuYENGsRqi3SEAwet';
 ...
 */
async function getClosedTrades(connection, wallet, startTime, endTime) {
    const walletKey = new web3_js_1.PublicKey(wallet);
    const programKey = new web3_js_1.PublicKey(exports.ADRENA_PROGRAM_ID);
    // Fetch up to 1000 signatures for this wallet
    const signatures = await connection.getSignaturesForAddress(walletKey, {
        limit: 1000,
    });
    // Filter to time window
    const relevant = signatures.filter((sig) => sig.blockTime !== null &&
    ...
    for (let i = 0; i < relevant.length; i += batchSize) {
        ...
        const txs = await connection.getParsedTransactions(batch.map((s) => s.signature), { maxSupportedTransactionVersion: 0 });
        for (const tx of txs) {
            if (!tx)
                continue;
...
function extractTradeFromTx(tx, wallet, programId) {
    ...
    const walletPreBalance = preBalances.find((b) => b.owner === wallet);
    const walletPostBalance = postBalances.find((b) => b.owner === wallet);
...
//# sourceMappingURL=trade-parser.js.map