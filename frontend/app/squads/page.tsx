...
import { MOCK_COMPETITION, MOCK_SQUADS } from "@/lib/mock-data";
...
export default function SquadsPage() {
  ...
  const filtered = MOCK_SQUADS.filter((s) =>
    ...
    <div className="page-container" style={{ maxWidth: 1100 }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 11, color: "var(--text-muted)", letterSpacing: "0.04em" }}>
            SEASON 1 · ROUND {MOCK_COMPETITION.roundNumber} · {MOCK_COMPETITION.totalSquads} SQUADS
          </p>
        </div>
      ...
      >
        <div>
          ...
          >
            ROUND {MOCK_COMPETITION.roundNumber} ENDS IN
          </div>
          <CompetitionTimer endTime={MOCK_COMPETITION.endTime} />
        </div>
        ...
        <div style={{ display: "flex", gap: 1 }}>
          {(["list", "grid"] as const).map((v) => (
            <button
              ...
              style={{
                padding: "6px 12px",
                backgroundColor: view === v ? "var(--surface-raised)" : "transparent",
                border: "1px solid var(--border)",
